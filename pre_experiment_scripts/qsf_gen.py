...
template = {"SurveyID":"SV_8nXoxz1CpoJ0wB0",
            "Element":"SQ",
            "PrimaryAttribute":"QID11",
            "SecondaryAttribute":"Test question",
            ...
            "Payload":
                {"QuestionText":"",
                 ...
                 "NextChoiceId":4,
                 "NextAnswerId":4,
                 "Answers": {"1":{"Display":"Explanation A"},
...
df = pd.read_excel("ads_explanations.xlsx")
...
for i, row in enumerate(df.itertuples()):
    ...
    qstring = (
        ...
        + middle_string
        + row[7].replace("```html", "").replace("```", "").replace("h1", "h3").replace("h2", "h3").replace("h4", "h3").replace("h5", "h3")
        + end_string
    )
    ...
    code = f"{int(row[2]):06}v{int(row[3]):06}"

    to_fill["Payload"]["DataExportTag"] = code
    to_fill["Payload"]["QuestionDescription"] = code
    to_fill["SecondaryAttribute"] = code

    blocks[2 + i] = {"Type": "Standard", "SubType": "", "Description": f"Block {2 + i}", "ID": f"BL_{''.join(random.choices(string.ascii_letters + string.digits, k=15))}", "BlockElements": [{"Type": "Question", "QuestionID": qid}]}

    # Append formatted JSON string to result
    result.append(json.dumps(to_fill, ensure_ascii=False))

# Write the result to a text file
with open("./ads_survey.qsf", "w", encoding="utf-8") as f:
    f.write(",\n".join(result) + ",")

# This should be added at the top of the qsf/txt file, as it indicates which questions are in which question blocks
print(str(blocks))
